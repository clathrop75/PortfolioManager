...
BEGIN
	SELECT (ey.EyRank + roa.RoaRank) AS CombinedRank, ey.EyRank, roa.RoaRank, c.CompanyName, ey.CompanyId, c.Symbol, ey.EarningsYield, roa.ReturnOnAssets FROM
...
BEGIN
	...
	WHERE b.YearEnding=varYear ORDER BY ReturnOnCapital DESC) AS roc
	ON ey.CompanyId=roc.CompanyId INNER JOIN Company c ON c.Id=ey.CompanyId
